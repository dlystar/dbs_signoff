...
export const EditableCell = ({ ...props }) => {
  ...
  const dateRange = setDateRange?.[rowNum]?.[filedCode]
  ...
  const { run, flush } = useDebounceFn(
    (fieldCode, rowNum, v, l) => {
      onCellChange(fieldCode || column?.key, rowNum, v, l);
    },
    {
      wait: 500,
    }
  ...
  let params = {
    ...
    utils,
    disabled,
    onBlur: (v, l) => flush(xProps?.fieldCode || column?.key, rowNum, v, l),
    onChange: (value, l) => {
      setVal(value);
      run(xProps?.fieldCode, row?.index, value, l);
    },
    changeOptions,
    tableKey,
    rowNum,
