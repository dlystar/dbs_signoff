...
import { signoffUpdate, signoffApproved, signoffRejected, signoffSendEmail, getSignOffListByWorkOrderId, signoffStatus, signoffInsertBatch, signoffDeleteBatch, addHistory } from '../api'
...
const Signoff = (props) => {
    ...
    useEffect(() => {
        if(styleDisplay == 'none' && editableStatus.includes(crStatus) && !formDisabled()){
            ...
            setTimeout(() => {
                const anchor = document.querySelector(`.LINK_BASIC_ProjectCutoverSignoff_${orderContainerID}`)
                console.log('anchor', anchor, `.LINK_BASIC_ProjectCutoverSignoff_${orderContainerID}`);
                if(anchor){
    ...
    const sendEmail = (rowNum) => {
        ...
        Modal.confirm({
            ...
            onOk() {
                const {workOrderId, nodeItem} = initData || {};
                const {nodeId, nodeName} = nodeItem || {};
                let params = {
                    workOrderId,
                    nodeId,
                    nodeName,
