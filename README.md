...
const OtherSignoff = (props) => {
  ...
  useEffect(() => {
    setSignoffTypeOptions([
      ...
      {
        "label": "Production Data Download (DCON)",
        "value": "DCON Signoff"
      },
      {
      ...
      if(includes(rowData.signOffType, 'Technical Live Verification (LV) Signoff', 'Business Live Verification (LV) Signoff', 'Implementation Checker Signoff')){
        if(crStatus == "Approved"){
          return {
            showStatus: false,
            showError: true,
            errorText: 'Required before close the CR'
          }
        }else{
          return {
            showStatus: false,
        ...
        return {
          showStatus: !includes(rowData.signOffType, 'DCON Signoff', 'Technical Live Verification (LV) Signoff', 'Business Live Verification (LV) Signoff', 'Implementation Checker Signoff'),
          showError: true,
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    render: (_, row) => {
                      ...
                      >
                        <SignOffUserGroup type="otherSignoff" row={row} disabled={formDisabled() || disabled} selectWidth="330px"/>
                      </Form.Item>
                    }
                  },
                  {
                  ...
                  },
                  {
                    ...
                    render: (_, row) => {
                      ...
                      // 除了这些signoff，其他都不需要附件
                      if(![...needArtefactSignoffs, ...shoowArtefactSignoffs].includes(rowData.signOffType?.[0])){
                        return null
                      }
                      return <Form.Item
