      ...
      {disabled ? (
        ...
        <DatePicker
          ...
          onOk={(date) => {
            ...
            // 如果存在禁用时间配置
            if (disabledTimes && disabledTimes.disabledHours) {
              // 获取被禁用的小时列表
              ...
              if (disabledHours.includes(currentHour)) {
                // 获取下一个可用的小时
                const nextHour = getNextAvailableHour(disabledHours, currentHour);
                if (nextHour === undefined) {
                  handleChange(value); // 保持原值
                  return;
                }
                date = date.clone().hours(nextHour);
              }


              // 获取被禁用的分钟列表
              const disabledMinutes = disabledTimes.disabledMinutes(date.hours());
              ...
              if (disabledMinutes.includes(currentMinute)) {
                // 获取下一个可用的分钟
                const nextMinute = getNextAvailableMAndS(disabledMinutes, currentMinute);
                if (nextMinute === undefined) {
                  handleChange(value); // 保持原值
                  return;
                }
                date = date.clone().minutes(nextMinute);
              }


              // 获取被禁用的秒数列表
              const disabledSeconds = disabledTimes.disabledSeconds(date.hours(), date.minutes());
              const currentSecond = date.seconds();


              // 如果当前秒数被禁用
              if (disabledSeconds.includes(currentSecond)) {
                // 获取下一个可用的秒数
                const nextSecond = getNextAvailableMAndS(disabledSeconds, currentSecond);
                if (nextSecond === undefined) {
                  handleChange(value); // 保持原值
                  return;
                }
                date = date.clone().seconds(nextSecond);
              }
            }
